...
async function syncWithRpc() {
  console.log('💾 Syncing via RPC function...\n')
  
  try {
    console.log(`📊 Total days to sync: ${allDays.length}`)
    console.log('\n📅 Friday Feb 20 (to be saved):')
    const friday = allDays.find(d => d.date === 'Vendredi 20 février')
    console.log(JSON.stringify(friday, null, 2))
    
    // Call the RPC function
    console.log('\n🔄 Calling RPC: update_page_content_data...')
