...
async function sync() {
  console.log('💾 Syncing Careme 2026 data...\n')
  
  try {
    // Find Friday
    const friday = days.find(d => d.date === 'Vendredi 20 février')
    console.log('📅 Friday Feb 20 (BEFORE):\n', JSON.stringify(friday, null, 2))
    console.log(`\n📊 Total days to sync: ${days.length}`)
    ...
    if (updateError) {
      ...
      console.log('\n🔄 Attempting DELETE + INSERT...')
      
      // Try DELETE + INSERT instead
      await supabase
        .from('page_content')
        .delete()
        .eq('page_key', 'careme-2026')
    ...
    console.log('\n✅ Verification - Total days:', data.content?.days?.length || 0)
    const fridayAfter = data.content?.days?.find(d => d.date === 'Vendredi 20 février')
    if (fridayAfter) {
      console.log('\n📅 Friday Feb 20 (AFTER):\n', JSON.stringify(fridayAfter, null, 2))
    }
  } catch (err) {
